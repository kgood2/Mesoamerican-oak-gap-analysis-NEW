...
main_dir_HLZ <- "/Volumes/GoogleDrive/Shared drives/Global Tree Conservation Program/4. GTCP_Projects/Gap Analyses/Conservation Gap Analysis - MESOAMERICAN OAKS/HLZ_Climate Scenarios /2061_2080_30seconds"
...
target_taxa <- target_taxa[!target_taxa %in% c("Quercus hinckleyi", "Quercus robusta", "Quercus tardifolia",
...
# Filter out taxa you do not want to include
target_files <- target_files[!target_taxa %in% c("Quercus hinckleyi", "Quercus robusta", "Quercus tardifolia",
                                               "Quercus edwardsiae", "Quercus alpescens", "Quercus furfuracea")]
...
ecoregions <- vect(file.path(main_dir_HLZ, "ssp245","EC-Earth3-Veg","HLZ_Earth3_Veg.shp"))
...
## write summary table
summary_tbl
write.csv(summary_tbl, file.path(main_dir_HLZ, "ssp245","EC-Earth3-Veg",
...
### PART B ####
################################################################################
#Run this part of the script to identify HLZs that are found in both current and 
# future scenarios, and count them. FIRST you need to create a new csv file with 
# the results from the part of the script above. Name it "HLZ_coverage" and save
# it in the right folder
################################################################################

HLZ <- read.csv(file.path(main_dir_HLZ, "ssp245","EC-Earth3-Veg", "HLZ_coverage.csv"), 
                header=T, colClasses="character",na.strings=c("","NA"))
...
find_conserved <- function(hlz_insitu, hlz_climatechange) {
  insitu_numbers <- strsplit(trimws(hlz_insitu), ",")[[1]]
  climatechange_numbers <- strsplit(trimws(hlz_climatechange), ",")[[1]]
  insitu_numbers <- as.numeric(insitu_numbers)
  climatechange_numbers <- as.numeric(climatechange_numbers)
  conserved <- intersect(insitu_numbers, climatechange_numbers)
  return(paste(conserved, collapse = ","))
}

HLZ <- HLZ %>%
  mutate(conserved = mapply(find_conserved, hlz_insitu, hlz_climatechange))


# add a new count_conserved column that counts the number of HLZs in the conserved column
HLZ$count_conserved <- sapply(strsplit(as.character(HLZ$conserved), ","), function(x) length(unique(x)))


#write csv
write.csv(HLZ, file.path(main_dir_HLZ, "ssp245","EC-Earth3-Veg",
                         paste0("HLZ_Conserved_ClimateChange",Sys.Date(),".csv")),
          row.names = F)  
