...
redlist_raw <- redlist_raw %>%
  select(taxon_name,sci_name,tax_comm,event_year,
...
# use only if you have multiple genera
...
#read in data
#use this code if you have only one genus (e.g., Quercus)
sernec_raw <- read.csv(file.path(main_dir, data, raw, "NorthAm_herbaria", "occurrences.csv"), colClasses = "character",
                       na.strings=c("", "NA"), strip.white=T, fileEncoding="UTF-8")
nrow(sernec_raw)
