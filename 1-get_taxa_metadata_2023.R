...
# read in taxa list
taxon_list <- read.csv(file.path(main_dir,"inputs","taxa_list","taxa_all.csv"), 
                       header=T, colClasses="character",na.strings=c("","NA"))

#remove taxa that you do not want to include 
...
if(!dir.exists(file.path(main_dir, "inputs", output_dir)))
  dir.create(file.path(main_dir, "inputs", output_dir), recursive=T)
...
category <- read.csv(file.path(main_dir,"inputs", "known_distribution",
...
add_manually <- data.frame(
  taxon_name_accepted = c("Quercus clivicola","Quercus cordifolia", "Quercus eugeniifolia",
                          "Quercus flagellifera", "Quercus huicholensis","Quercus pennivenia",
                          "Quercus seemannii","Quercus tristis"),
  manual_native_dist = c("Mexico","Mexico","Costa Rica",
                         "Guatemala","Mexico","Mexico",
                         "Mexico; Costa Rica; El Salvador; Guatemala; Honduras; Nicaragua; Panama","Mexico" ),
  manual_native_dist_iso2 = c("MX","MX","CR",
                              "GT","MX","MX",
                              "MX; CR; SV; GT; HN; NI; PA", "MX"))
...
# write file
write.csv(taxon_list, file.path(main_dir, "inputs", "taxa_metadata",
                                "target_taxa_with_synonyms_for_richness.csv"), row.names=F)